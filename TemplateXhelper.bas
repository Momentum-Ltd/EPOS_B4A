...
private Sub InitializeLocals 'ignore (compiler warning)
	progressbox.Initialize(Me, "progressbox", modEposApp.DFT_PROGRESS_TIMEOUT)
End Sub

' Show the process box
Private Sub ProgressHide 'ignore (compiler warning)
	progressbox.Hide
End Sub

' Hide The process box.
Private Sub ProgressShow(message As String) 'ignore (compiler warning)
