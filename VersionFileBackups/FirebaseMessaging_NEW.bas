...
#Region  Documentation
	...
	' Amendee...: D Morris.
	' Details...:  Added: AquireNewFcmToken(). 
	'
	' Date......: 
	' Release...: 
	' Overview..:
...
Sub Process_Globals
	...
	Private refreshTriggered As Boolean = False
...
Sub fm_TokenRefreshed(newToken As String)
	...
	'TODO Code required to update customer info on the Web Server.
...
' Requests the FCM Server to invoke the process to generate a new FCM Token for this device.
'  Warning New FCM token is not available until the fm_TokenRefreshed event triggers.
Public Sub AquireNewFcmToken As ResumableSub
	Dim newToken As String = ""
	refreshTriggered = False
	Log("Token:" & fm.Token)
	fm.UnsubscribeFromTopic("general") 'unsubscribe from topics
	Sleep(1000)
	SubscribeToTopics
	Log("Token:" & fm.Token)
	Do While refreshTriggered = False
		Sleep(100)
	Loop
	newToken = GetFcmToken
	Return newToken
End Sub
