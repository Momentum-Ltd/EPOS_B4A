...
'	Private mEndOnPause As Boolean = False 		' Stores whether the activity should be closed the next time it is Paused.
	...
	' View declarations
	Private btnAddUpdateItem As B4XView 		' The button which adds/updates the selected item on the order's item list.
	Private btnDecQty As B4XView 				' The button which decrements the item quantity.
	Private btnDelete As B4XView 				' The button which deletes the currently-editing item from the order.
	...
	Private btnMoreSizes As B4XView				' The button to show more sizes options list.
#End If
	Private btnTopLevel As B4XView 				' The button which returns the user to the top (Main Category) level of the selection tree.
	Private btnUpOneLevel As B4XView 			' The button which returns the user to the previous selection level.
...
#End If
		...
		Case LEVEL_GOODSDESC
			...
			Next
			btnAddUpdateItem.Visible = False
