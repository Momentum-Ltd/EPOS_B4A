...
#Region Documentation
	...
	' Date......: 10/02/21
	' Release...: 94
	' Overview..: Communications methods moved to their own class.
	' Amendee...: D Morris
	' Details...: Mod: Methods moved to clsCommunications class.
	' 			  			    
	' Date......: 
	' Release...: 
	' Overview..: 
...
'	Public connect As clsConnect 						' Move to clsCommunications -  Object which handles TCP socket operations.
	...
	Public DisconnectedCloseActivities As Boolean 		' Remove later -  Whether all activities (other than Connection) should be ended due to a disconnect command.
	...
	Public LogFile As clsReportErrors					' Error logging
...
#if INCLUDE_SOCKET_CODE
	Public Astreams As AsyncStreams 			' The asynchronous streams object used to read and write socket transmissions.
	Public CltSock As Socket 					' The socket object which handles the connection to the Server.
#end if	
...
'	connect.Initialize ' Initialise the connection checking.
	...
	comms.Initialize ' Need to be at end to ensure settings have been setup.
	Log("TEST")
End Sub
...
'	lAppendToExtLog(ERROR_LIST_FILENAME, "An unhandled exception occurred: " & CRLF & StackTrace) ' Log the exception
	LogFile.LogReport(modEposApp.ERROR_LIST_FILENAME, "An unhandled exception occurred: " & CRLF & StackTrace) ' Log the exception
...
Private Sub AStreams_NewData(Buffer() As Byte)
	...
	comms.connect.RetriggerServerCheck
	If tempResponse.Contains(modEposApp.EOF_STRING) Then
		...
		inputStrg = inputStrg.Replace(modEposApp.HEADER_STRING, "").Replace(modEposApp.EOF_STRING, "")
		comms.ProcessInputStrg(inputStrg)
...
Public Sub IncrementDisconnectCounter
	...
	If disconnectCounter = DISCONNECT_THRESHOLD Then
		...
		comms.connect.StopServerChecking ' Stop the regular server pings (as they will cause reconnection)
...
'	End Try
	...
	comms.ProcessInputStrg(inputCommsStrg)
...
'	Return statusCode
	...
	wait for (comms.SendMessage(msg)) complete(statusCode As Int)
	Return statusCode
End Sub
...
Public Sub StopAntiSleepPhoneModes
...
Private Sub TruncateCommsBuffer(inputStr As String) As String
	Dim firstMsgEndIndex As Int = inputStr.IndexOf(modEposApp.EOF_STRING) + modEposApp.EOF_STRING.Length
	...
	If inputStr.Length > firstMsgEndIndex Then ' Message has been truncated - log the buffer data
		LogFile.LogReport( modEposApp.ERROR_COMMS_FILENAME, "The comms buffer was truncated, from its full contents:" & CRLF & inputStr)
	End If
