...
#Region  Documentation
	...
	' Amendee...: D Morris
	' Details...: Added: DisplayAllCentres() and GetNearbyCentres().
	'
	' Date......: 03/02/21
	' Release...: 13
	' Overview..: More methods added.
	' Amendee...: D Morris
	' Details...: Added: IsCentreOpen() and WebSignedOntoCentre().
	'             Rename: DisplayAllCentres() to GetAllCentres().
	'			  Rename: GetNearbyCentres() to GetNearbyCentres().
	' 			  Rename: IncrementCustomerIdRevision() to NewCustomerIdRevision().
	'
	' Date......: 
	' Release...: 
	' Overview..:
...
' Downloads from the Web Server a list of all centres.
'   returns rxMsg if download ok else null if error.
Public Sub GetAllCentres() As ResumableSub
	Dim rxMsg As String = ""
...
' Downloads fromt he Web Server a list of nearby centres.
'  returns rxMsg if download ok else null if error.
Public Sub GetNearbyCentres(pCurrentLocation As Location) As ResumableSub
	Dim rxMsg As String = ""
...
' Generates a new customer ID revision.
' returns new apiCustomerId if email and password are ok (else -1: both wrong or 0: email ok).
Public Sub NewCustomerIdRevision(email As String, password As String) As ResumableSub
...
public Sub IsCentreOpen(centreId As Int) As ResumableSub
...
Public Sub WebSignedOntoCentre As ResumableSub
