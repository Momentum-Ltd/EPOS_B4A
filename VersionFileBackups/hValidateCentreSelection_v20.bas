...
#Region  Documentation
	...
	' Amendee...: D Morris
	' Details...:   Mod: lWebSignedOntoCentre() now uses data from myData.
	'			    Mod: HandleConnectToServerResponse() code moved to new HandleOpenTabConfirmResponse().
	'			  Added: HandleOpenTabConfirmResponse().
	'
	' Date......: 22/12/19
	' Release...: 10
	' Overview..: Saves selected centre details.
	' Amendee...: D Morris.
	' Details...: Mod: btnYes_Click() updates starter.myData.Centre information.
		'
	' Date......: 29/12/19
	' Release...: 11
	' Overview..: More centre information is stored in myData when selected.
	' Amendee...: D Morris
	' Details...: Mod: btnYes_Click() stores postcode.
	'
	' Date......: 21/03/20
	' Release...: 12
	' Overview..: #315 Issue removed B4A compiler warnings. 
	' Amendee...: D Morris
	' Details...:  Mod: lCheckWifiPingServer()not used removed.
	'			   Mod: MainValidateCentreSelection() msgbox() replaced with msgboxasync().
	'			
	' Date......: 26/04/20
	' Release...: 13
	' Overview..: Bug #0186: Problem moving accounts support for new customerId (with embedded rev).
	' Amendee...: D Morris.
	' Details...:  Mod: lWebSignedOntoCentre(), HandleConnectToServerResponse().
	'
	' Date......: 09/05/20
	' Release...: 14
	' Overview..: Bugfix: 0398 - Timeout sometimes tripping at startup.
	' Amendee...: D Morris.
	' Details...:  Mod: HandleConnectToServerResponse(), lStartSignOnToCentre() restart timer.
	'			   Mod:	HandleOpenTabConfirmResponse(), MainValidateCentreSelection() stops timer.
	'              Remarks: The timer handle is now correctly restarted (using false then true) for both
	'				but open tab and table confirmation oherwise unable to see any problems.
	'				Also increased the timeout to 15secs to help (not this may exceed the progressDialog timeout. 
	'
	' Date......: 11/05/20
	' Release...: 15
	...
	' Amendee...: D Morris.
	' Details...:  Mods: lStartSignOnToCentre() - references to Wifi removed.
	'			   Mods: HandleConnectToServerResponse() - now exits to Sync Data or Select Centre.
	'			   Mods: HandleConnectToServerResponse() now has a signon parameter.
	'
	' Date......: 11/06/20
	' Release...: 17
	' Overview..: Mod: Support for second Server.
	' Amendee...: D Morris.
	' Details...:  Mod: lWebSignedOntoCentre().
	'
	' Date......: 18/06/20
	' Release...: 18
...
'		End If
		...
		btnNo.Visible = True ' Wait for customer to confirm or reject centre.
		btnYes.Visible = True
	Else 
