...
'	Private txtAuthorisePw As B4XView
	' Enter details panel
	Private btnClear As B4XView			' Clear displayed information button
	Private btnSubmit As B4XView		' Submit information button
...
Sub btnSubmit_Click
	If pnlEnterDetails.Visible = True Then ' Only works if edit details panel shown.
		...
		If infoOk Then
			btnClear.Visible = False ' Switches to the authorisation panel to enter password
...
private Sub InitializeLocals
	If progressbox.IsInitialized = False Then
		progressbox.Initialize(Me, "progressbx",modEposApp.DFT_PROGRESS_TIMEOUT) 		
	End If
End Sub
