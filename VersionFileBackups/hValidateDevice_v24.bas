...
#Region  Documentation
	...
	'			  Mod: Rename lncCustomerRev() to NewCustomerRev().
	'
	' Date......: 
	' Release...: 
	' Overview..:
...
' Generates a new customer ID revision number.
' returns new customerId (with embedded rev) if email and password are ok (else -1 error).
Private Sub NewCustomerRev(email As String, password As String) As ResumableSub
	Wait for (apiHelper.NewCustomerIdRevision(email, password)) complete (customerId As Int)
	Return customerId
End Sub
...
Private Sub ValidateTheDevice
	...
	If emailAddress <> "" And password <> "" Then
		If modEposApp.CheckEmailFormat(emailAddress) Then ' Password format ok?
			Wait For (NewCustomerRev(emailAddress, txtPassword.Text.Trim)) complete (apiCustomerId As Int)
