...
'	Public location As clsLocation						' Location services (FLP).
...
'	location.Initialize("Starter", "location") ' Location services.
...
Sub Service_Start(StartingIntent As Intent)
	' Currently nothing
	' Not sure but line below is used by FLP example.
...
Private Sub lHandleCustomerDetailsMsg(openTabCmdResponseStr As String)
	...
	CallSubDelayed2(ValidateCentreSelection, "ConnectToServerResponse", centreSignonOk)
End Sub
	...
	If customerDetailsObj.authorized Then
		If customerDetailsObj.customerId = myData.customer.customerId Then ' Customer ID correct?
			If customerDetailsObj.centreId = myData.centre.centreId Then ' Centre ID correct?
				...
				CallSubDelayed(ValidateCentreSelection, "OpenTabConfirmResponse")
			End If
		End If		
	End If
End Sub
...
Private Sub ShowMessageNotification(headingTop As String)
	...
	else if IsPaused(aShowOrderStatusList) = False Then
		CallSubDelayed2(aShowOrderStatusList, "ShowMessageNotificationMsgBox", True)
...
Private Sub ShowStatusNotification
	...
	If IsPaused(aTaskSelect) = False Then ' Task Select activity is active - show the message directly
		...
		CallSubDelayed2(aShowOrderStatusList, "ShowStatusNotificationMsgBox", True)
	else if IsPaused(aPlaceOrder) = False Then
